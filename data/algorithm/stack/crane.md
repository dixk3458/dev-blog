...
크레인을 모두 작동시킨 후 게임규칙에 따라 터트려 없앤 인형의 개수를 반환하는 문제이다.

### 전략 🔧

그림과 크레인을 고려했을때 LIFO방식의 Stack 유형 문제임을 알 수 있다.

1. moves 배열을 순회하면서 board에 있는 인형을 Stack에 넣어주자.
   1. moves는 1부터 시작하지만, board는 0부터 인덱스가 시작하는것을 유의하자.
...
function solution(board, moves) {
  ...
  for (let my of moves) {
    // board의 my - 1열을 탐색하자.
    // 즉 my - 1열을 고정하고 board.length - 1행부터 시작해 0번째 행까지 탐색
    // 요소를 만났으면 pop하고 그것을 stack에 있는 top요소에 따라 넣을지 말지 결정

    for (let i = 0; i < length; i++) {
      if (board[i][my - 1]) {
        const popped = board[i][my - 1];
        board[i][my - 1] = 0;

        if (stack[stack.length - 1] === popped) {
          answer = answer + 2;
          stack.pop();
        } else {
          stack.push(popped);
